...
## 共通ルール（webセッションを含む全セッションが従う）
- 調査・確認・レビュー・提案だけの依頼では、対象ファイルを変更せず、commit・push・デプロイもしない。証拠と未確認事項を報告する。
- 実装・文書更新では、依頼範囲の変更と変更に見合う検証を行う。今回の変更が原因の失敗を修正して関連チェックを再実行し、根拠なく検証範囲を広げない。
- 公開やpushは、このファイルのプロジェクト固有の運用と `DEPLOY.md` に従う。文書・ルールだけの変更は自動デプロイの対象にしない。
- 秘密情報（APIキー・トークン・パスワード・`.env` の中身）を表示・保存しない。見つけたら値を出さず場所だけ報告する。
- 実装・文書更新の完了時、または明示的な引き継ぎ依頼時に、必要な変更があれば HANDOFF.md を更新する。調査・確認・レビューだけでは更新しない。
- 共通Skillは `~/.agents/skills/` の正本に従う。使うのはタスクが発火条件を満たすSkillだけとする。
