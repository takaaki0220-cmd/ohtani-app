...
- 調査・確認の依頼では修正せず、調査結果の報告のみ。修正は指示があってから。
- 変更は最小限。指示範囲外のリファクタリング・整形はしない。
- push は本番反映につながる操作として扱う。コード実装後に「差分レビュー → 秘密情報チェック → ビルド確認」を行い、すべて通れば確認を挟まず `main` へ push して自動デプロイする。完了後は公開URLをリンクで報告する。
- 秘密情報（APIキー・トークン・.env の中身）を表示・保存しない。
- 実装・文書更新の完了時、または明示的な引き継ぎ依頼時に、必要な変更があれば HANDOFF.md を更新する（やったこと / いまの状態 / 次の一歩 / 検証結果）。調査・確認・レビューだけの依頼ではファイルを更新せず、チャットで報告する。
- ローカル（Mac）のセッションでは ~/.agents/skills/ の共通スキルにも従う。
